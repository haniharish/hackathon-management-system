...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-forest">
      <motion.header initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8 }} className="sticky top-0 z-50 border-b border-white/40 bg-white/40 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 rounded-full border border-white/60 bg-white/70 px-5 py-3 shadow-lg backdrop-blur md:flex">
            {navItems.map((item) => (
      ...
      <footer className="mx-auto mt-16 max-w-7xl px-6 pb-10 text-sm text-forest/70 lg:px-8">
        <div className="rounded-[2rem] border border-white/60 bg-white/50 p-6 shadow-xl backdrop-blur">
